-- TODO: replace <leader> to <Leader> everywhere in KickstartNvim v4 to match vimdoc
local utils = require "kickstart.utils"
local ui = require "kickstart.utils.ui"

local get_icon = utils.get_icon
local is_available = utils.is_available

local maps = require("kickstart.utils").empty_map_table()
...
-- [[ KickstartNvim ]]
maps.n["<leader>pa"] = { "<cmd>KickstartUpdatePackages<cr>", desc = "Update Plugins and Mason Packages" }
maps.n["<leader>pA"] = { "<cmd>KickstartUpdate<cr>", desc = "KickstartNvim Update" }
maps.n["<leader>pv"] = { "<cmd>KickstartVersion<cr>", desc = "KickstartNvim Version" }
maps.n["<leader>pl"] = { "<cmd>KickstartChangelog<cr>", desc = "KickstartNvim Changelog" }
...
maps.n["<leader>bc"] =
  { function() require("kickstart.utils.buffer").close_all(true) end, desc = "Close all buffers except current" }
maps.n["<leader>bC"] = { function() require("kickstart.utils.buffer").close_all() end, desc = "Close all buffers" }
maps.n["<leader>bb"] = {
  function()
    require("kickstart.utils.status.heirline").buffer_picker(function(bufnr) vim.api.nvim_win_set_buf(0, bufnr) end)
  end,
  desc = "Select buffer from tabline",
}
maps.n["<leader>bd"] = {
  function()
    require("kickstart.utils.status.heirline").buffer_picker(
      function(bufnr) require("kickstart.utils.buffer").close(bufnr) end
    )
  end,
  desc = "Close buffer from tabline",
}
maps.n["<leader>bl"] =
  { function() require("kickstart.utils.buffer").close_left() end, desc = "Close all buffers to the left" }
maps.n["<leader>bp"] = { function() require("kickstart.utils.buffer").prev() end, desc = "Previous buffer" }
maps.n["<leader>br"] =
  { function() require("kickstart.utils.buffer").close_right() end, desc = "Close all buffers to the right" }

maps.n["<leader>b\\"] = {
  function()
    require("kickstart.utils.status.heirline").buffer_picker(function(bufnr)
      vim.cmd.split()
...
-- [[ Manage Buffers ]] --
maps.n["<leader>c"] = { function() require("kickstart.utils.buffer").close() end, desc = "Close buffer" }
maps.n["<leader>C"] = { function() require("kickstart.utils.buffer").close(0, true) end, desc = "Force close buffer" }
maps.n["]b"] =
  { function() require("kickstart.utils.buffer").nav(vim.v.count > 0 and vim.v.count or 1) end, desc = "Next buffer" }
maps.n["[b"] = {
  function() require("kickstart.utils.buffer").nav(-(vim.v.count > 0 and vim.v.count or 1)) end,
  desc = "Previous buffer",
}
maps.n[">b"] = {
  function() require("kickstart.utils.buffer").move(vim.v.count > 0 and vim.v.count or 1) end,
...
-- [[
--#############################
-- SECTION: SORTING BUFFERS
--#############################
--]]
maps.n["<leader>bs"] = sections.bs
maps.n["<leader>bse"] = { function() require("kickstart.utils.buffer").sort "extension" end, desc = "By extension" }
maps.n["<leader>bsr"] =
  { function() require("kickstart.utils.buffer").sort "unique_path" end, desc = "By relative path" }
maps.n["<leader>bsp"] = { function() require("kickstart.utils.buffer").sort "full_path" end, desc = "By full path" }
maps.n["<leader>bsi"] = { function() require("kickstart.utils.buffer").sort "bufnr" end, desc = "By buffer number" }
maps.n["<leader>bsm"] = { function() require("kickstart.utils.buffer").sort "modified" end, desc = "By modification" }
...
-- [[ Noice ]] --
if is_available "noice.nvim" then maps.n["<leader>sn"] = { name = "+noice" } end
...
if is_available "toggleterm.nvim" then
  ...
  if vim.fn.executable "lazygit" == 1 then
    ...
    maps.n["<leader>gg"] = {
      function()
        local worktree = require("kickstart.utils.git").file_worktree()
  ...
  if vim.fn.executable "byptop" == 1 then
    maps.n["<leader>tt"] = { function() utils.toggle_term_cmd "byptop" end, desc = "ToggleTerm byptop" }
  end
  ...
  maps.n["<C-'>"] = maps.n["<F12>"] -- requires terminal that supports binding <C-'>
  maps.t["<C-'>"] = maps.n["<F12>"] -- requires terminal that supports binding <C-'>
end
...
if is_available "mini.files" then
  maps.n["<leader>e"] = {
    function()
      ...
      local explorer = mini.get_target_window()
...
if is_available "telescope.nvim" and require("telescope").extensions.file_browser ~= nil then
  maps.n["<leader>e"] =
    { ":Telescope file_browser<CR>", desc = "Toggle Telescope file_browser (directory of current file)" }
  maps.n["<leader>o"] =
    { ":Telescope file_browser path=%:p:h select_buffer=true<CR>", desc = "Toggle Telescope file_browser (cwd)" }
end
...
if is_available "auto-session" then
  ...
  maps.n["<leader>Ss"] = { "<cmd>:SessionSave<cr>", desc = "Save this session" }
  maps.n["<leader>Sl"] = { "<cmd>:SessionRestore<cr>", desc = "Load last session" }
  maps.n["<leader>Sd"] = { "<cmd>:Autosession delete<cr>", desc = "Delete session" }
  maps.n["<leader>S."] = { "<cmd>:SessionRestore<cr>", desc = "Load current directory session" }
  maps.n["<leader>Sf"] = { require("auto-session.session-lens").search_session, desc = "Search sessions" }
end
...
if is_available "telescope.nvim" then
  ...
  maps.n["<leader>fa"] = {
    function()
      ...
      local search_dirs = {}
      for _, dir in ipairs(kickstart.supported_configs) do -- search all supported config locations
        if dir == kickstart.install.home then dir = dir .. "/lua/user" end -- don't search the kickstart core files
    ...
    desc = "Find KickstartNvim config files",
  }
  ...
  maps.n["<leader>mf"] = { function() require("telescope.builtin").man_pages() end, desc = "Find man" }
...
utils.set_mappings(kickstart.user_opts("mappings", maps))
