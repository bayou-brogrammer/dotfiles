...
local kickstartevent = utils.event
...
autocmd("BufEnter", {
  desc = "Quit KickstartNvim if more than one window is open and only sidebar windows are list",
...
autocmd("ColorScheme", {
  ...
  group = augroup("kickstartnvim_highlights", { clear = true }),
  callback = function()
    if vim.g.colors_name then
      for _, module in ipairs { "init", vim.g.colors_name } do
        for group, spec in pairs(kickstart.user_opts("highlights." .. module)) do
          vim.api.nvim_set_hl(0, group, spec)
        end
      end
    end

    kickstartevent("ColorScheme", false)
  end,
})

autocmd({ "BufReadPost", "BufNewFile", "BufWritePost" }, {
  desc = "KickstartNvim user events for file detection (KickstartFile and KickstartGitFile)",
...
cmd("KickstartUpdate", function() require("kickstart.utils.updater").update() end, { desc = "Update KickstartNvim" })
cmd(
  "KickstartVersion",
