--- ### KickstartNvim Core Bootstrap
--
-- This module simply sets up the global `kickstart` module.
-- This is automatically loaded and should not be resourced, everything is accessible through the global `kickstart` variable.
--
-- @module kickstart.config.bootstrap
-- @copyright 2022
...
kickstart.install = _G["kickstartnvim_installation"] or { home = vim.fn.stdpath "config" }
kickstart.supported_configs = { kickstart.install.home }

--- external kickstart configuration folder
kickstart.install.config = vim.fn.stdpath("config"):gsub("[^/\\]+$", "kickstart")

-- check if they are the same, protects against NVIM_APPNAME use for isolated install
if kickstart.install.home ~= kickstart.install.config then
  vim.opt.rtp:append(kickstart.install.config)
  --- supported kickstart user conifg folders
  table.insert(kickstart.supported_configs, kickstart.install.config)
end
...
local function load_module_file(module)
  ...
  for _, config_path in ipairs(kickstart.supported_configs) do
...
function kickstart.user_opts(module, default, extend)
...
kickstart.updater = {
  options = kickstart.user_opts("updater", { remote = "origin", channel = "stable" }),
...
local options = kickstart.updater.options
if kickstart.install.is_stable ~= nil then options.channel = kickstart.install.is_stable and "stable" or "nightly" end
if options.pin_plugins == nil then options.pin_plugins = options.channel == "stable" end

--- table of user created terminals
kickstart.user_terminals = {}
--- table of language servers to ignore the setup of, configured through lsp.skip_setup in the user configuration
kickstart.lsp = { skip_setup = kickstart.user_opts("lsp.skip_setup", {}), progress = {} }
--- the default colorscheme to apply on startup
kickstart.default_colorscheme = kickstart.user_opts("colorscheme", "nightfox", false)
