--- ### KickstartNvim Utilities
--
-- Various utility functions to use within KickstartNvim and user configurations.
--
-- This module can be loaded with `local utils = require "kickstart.utils"`
--
-- @module kickstart.utils
-- @copyright 2022
...
function M.notify(msg, type, opts)
  vim.schedule(function() vim.notify(msg, type, M.extend_tbl({ title = "KickstartNvim" }, opts)) end)
end
...
function M.event(event, delay)
  local emit_event = function() vim.api.nvim_exec_autocmds("User", { pattern = "Kickstart" .. event, modeline = false }) end
  if delay == false then
    emit_event()
  else
...
function M.toggle_term_cmd(opts)
  local terms = kickstart.user_terminals
...
--- Partially reload KickstartNvim user settings. Includes core vim options, mappings, and highlights. This is an experimental feature and may lead to instabilities until restart.
...
function M.reload(quiet)
  ...
  local core_modules = { "kickstart.config.bootstrap", "kickstart.config.options", "kickstart.config.mappings" }
...
function M.get_icon(kind, padding, no_fallback)
  ...
  if not M[icon_pack] then
    M.icons = kickstart.user_opts("icons", require "kickstart.icons.nerd_font")
    M.text_icons = kickstart.user_opts("text_icons", require "kickstart.icons.text")
  end
  local icon = M[icon_pack] and M[icon_pack][kind]
  return icon and icon .. string.rep(" ", padding or 0) or ""
end

--- Get a icon spinner table if it is available in the KickstartNvim icons. Icons in format `kind1`,`kind2`, `kind3`, ...
