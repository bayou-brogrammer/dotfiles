...
--
-- KickstartNvim Updater utilities to use within KickstartNvim and user configurations.
--
-- This module can also loaded with `local updater = require("kickstart.utils.updater")`
--
-- @module kickstart.utils.updater
-- @see kickstart.utils
-- @copyright 2022
...
--- Helper function to generate KickstartNvim snapshots (For internal use only)
...
function M.generate_snapshot(write)
  local file
  local prev_snapshot = require(kickstart.updater.snapshot.module)
  ...
  if write == true then
    file = assert(io.open(kickstart.updater.snapshot.path, "w"))
    file:write "return {\n"
  end
...
function M.version(quiet)
  local version = kickstart.install.version or git.current_version(false) or "unknown"
  if kickstart.updater.options.channel ~= "stable" then version = ("nightly (%s)"):format(version) end
...
--- Create a table of options for the currently installed KickstartNvim version
...
function M.create_rollback(write)
  ...
  if write == true then
    local file = assert(io.open(kickstart.updater.rollback_file, "w"))
...
function M.rollback()
  local rollback_avail, rollback_opts = pcall(dofile, kickstart.updater.rollback_file)
...
function M.update_available(opts)
  if not opts then opts = kickstart.updater.options end
