--- ### Git LUA API
--
-- This module can be loaded with `local git = require "kickstart.utils.git"`
--
-- @module kickstart.utils.git
-- @copyright 2022
...
--- Run a git command from the KickstartNvim installation directory
---@param args string|string[] the git arguments
---@return string|nil # The result of the command or nil if unsuccessful
function git.cmd(args, ...)
  if type(args) == "string" then args = { args } end
  return require("kickstart.utils").cmd(vim.list_extend({ "git", "-C", kickstart.install.home }, args), ...)
end
...
--- Check if the KickstartNvim is able to reach the `git` command
...
--- Check if the KickstartNvim home is a git repo
...
---@return string|nil # The branch of the KickstartNvim installation
...
function git.parse_remote_url(str)
  return vim.fn.match(str, require("kickstart.utils").url_matcher) == -1
      and git.url .. str .. (vim.fn.match(str, "/") == -1 and "/KickstartNvim.git" or ".git")
    or str
end
...
---@param commits string[] an array like table of commit messages
---@return string[][] # An array like table of echo messages to provide to nvim_echo or kickstart.echo
