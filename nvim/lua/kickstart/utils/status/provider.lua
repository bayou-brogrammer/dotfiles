--- ### KickstartNvim Status Providers
--
-- Statusline related provider functions for building statusline components
--
-- This module can be loaded with `local provider = require "kickstart.utils.status.provider"`
--
-- @module kickstart.utils.status.provider
-- @copyright 2023
-- @license GNU General Public License v3.0

local M = {}

local condition = require "kickstart.utils.status.condition"
local env = require "kickstart.utils.status.env"
local status_utils = require "kickstart.utils.status.utils"
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.signcolumn }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.numbercolumn }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.foldcolumn }
-- @see kickstart.utils.status.utils.stylize
function M.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require "kickstart.utils.ffi" -- get KickstartNvim C extensions
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.tabnr() }
function M.tabnr()
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.spell() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.paste() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.macro_recording() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.showcmd() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.search_count() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.mode_text() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.percentage() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.scrollbar() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.close_button() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.filetype() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.filename() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.file_encoding() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.file_format() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.unique_path() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.file_modified() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.file_read_only() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.file_icon() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.git_branch() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.git_diff({ type = "added" }) }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.lsp_progress() }
-- @see kickstart.utils.status.utils.stylize
function M.lsp_progress(opts)
  local spinner = utils.get_spinner("LSPLoading", 1) or { "" }
  return function()
    local _, Lsp = next(kickstart.lsp.progress)
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.treesitter_status() }
-- @see kickstart.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("kickstart.utils.status").provider.str({ str = "Hello" }) }
-- @see kickstart.utils.status.utils.stylize
function M.str(opts)
