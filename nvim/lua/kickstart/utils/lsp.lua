--- ### KickstartNvim LSP Utils
--
-- LSP related utility functions to use within KickstartNvim and user configurations.
--
-- This module can be loaded with `local lsp_utils = require("kickstart.utils.lsp")`
--
-- @module kickstart.utils.lsp
-- @see kickstart.utils
-- @copyright 2022
...
local user_opts = kickstart.user_opts
...
M.setup_diagnostics = function(signs)
  local default_diagnostics = kickstart.user_opts("diagnostics", {
    virtual_text = true,
    signs = { active = signs },
...
M.setup = function(server)
  ...
  if not vim.tbl_contains(kickstart.lsp.skip_setup, server) and setup_handler then setup_handler(server, opts) end
end
...
--- The `on_attach` function used by KickstartNvim
...
M.on_attach = function(client, bufnr)
  local lsp_mappings = require("kickstart.utils").empty_map_table()
  ...
  if client.supports_method "textDocument/formatting" and not tbl_contains(M.formatting.disabled, client.name) then
    ...
    then
      ...
      lsp_mappings.n["<leader>uf"] = {
        function() require("kickstart.utils.ui").toggle_buffer_autoformat() end,
        desc = "Toggle autoformatting (buffer)",
      }
      lsp_mappings.n["<leader>uF"] = {
        function() require("kickstart.utils.ui").toggle_autoformat() end,
  ...
  if client.supports_method "textDocument/inlayHint" then
    ...
    if vim.lsp.inlay_hint then
      ...
      lsp_mappings.n["<leader>uH"] = {
        function() require("kickstart.utils.ui").toggle_buffer_inlay_hints(bufnr) end,
  ...
  if client.supports_method "textDocument/semanticTokens/full" and vim.lsp.semantic_tokens then
    if vim.g.semantic_tokens_enabled then
      ...
      lsp_mappings.n["<leader>uY"] = {
        function() require("kickstart.utils.ui").toggle_buffer_semantic_tokens(bufnr) end,
  ...
  for id, _ in pairs(kickstart.lsp.progress) do -- clear lingering progress messages
    if not next(vim.lsp.get_active_clients { id = tonumber(id:match "^%d+") }) then kickstart.lsp.progress[id] = nil end
  end
...
--- The default KickstartNvim LSP capabilities
    ...
    lsp_opts.before_init = function(param, config)
      if vim.b.neodev_enabled then
        for _, kickstartnvim_config in ipairs(kickstart.supported_configs) do
          if param.rootPath:match(kickstartnvim_config) then
            table.insert(config.settings.Lua.workspace.library, kickstart.install.home .. "/lua")
            break
          end
        end
      end
