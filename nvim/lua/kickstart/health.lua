...
function M.check()
	health.start "KickstartNvim"

	health.info("KickstartNvim Version: " .. require("kickstart.utils.updater").version(true))
	...
	local programs = {
		{
			...
			extra_check = function(program)
				local git_version = require("kickstart.utils.git").git_version()
				if git_version then
